...
OPENAI_AUDIO_MODEL = os.environ.get(
    "OPENAI_AUDIO_MODEL", openai_config.get("audio_model", "whisper-1")
)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        whatsapp_client = WhatsAppClient(
            base_url=WHATSAPP_BASE_URL, username=WHATSAPP_API_USER, password=WHATSAPP_API_PASSWORD
        )
        ...
        logger.info(
            f"LLM service initialized (model: {OPENAI_MODEL}, vision: {OPENAI_VISION_MODEL or 'same'}, audio: {OPENAI_AUDIO_MODEL})"
        )
        ...
        if CHAT_JID:
            ...
            database.add_chat(
                chat_jid=CHAT_JID, chat_name="Default Chat (from config)", is_manual=True
            )
...
async def sync_chats():
    ...
    try:
        ...
        for i, group in enumerate(groups, 1):
            ...
            chat_jid = group.get("JID") or group.get("jid") or group.get("id") or group.get("ID")
            chat_name = (
                group.get("Name")
                or group.get("name")
            ...
            if chat_jid:
                ...
                if existing:
                    ...
                    database.add_chat(chat_jid=chat_jid, chat_name=chat_name, is_manual=False)
        ...
        for i, chat in enumerate(all_chats, 1):
            chat_jid = chat.get("jid") or chat.get("JID") or chat.get("id") or chat.get("ID")

            # Skip if it's a group (already handled above)
            if chat_jid and "@g.us" in chat_jid:
                logger.debug(
                    f"  [{i}/{len(all_chats)}] Skipping group (already synced): {chat_jid}"
                )
                continue
...
async def add_chat(request: AddChatRequest):
    ...
    try:
        ...
        if not chat_name:
            logger.info(
                f"No name provided for {request.chat_jid}, attempting to fetch from WhatsApp API"
            )
            ...
            if chat_info:
                chat_name = (
                    chat_info.get("name")
                    or chat_info.get("subject")
                    or chat_info.get("title")
                    or f"Chat {request.chat_jid}"
                )
        ...
        success = database.add_chat(chat_jid=request.chat_jid, chat_name=chat_name, is_manual=True)
...
async def get_chat_messages(chat_jid: str, limit: int = 20):
    ...
    try:
        ...
        return {
            ...
            "results": {"chat_jid": chat_jid, "messages": messages, "count": len(messages)},
        }
    except Exception as e:
...
async def bulk_action_chats(chat_jids: List[str], action: str, bot_name: Optional[str] = None):
    """
    ...
    """
    try:
        results = {"success": [], "failed": [], "total": len(chat_jids)}
            ...
            try:
                if action == "start_bots":
                    ...
                    results["success"].append(
                        {"chat_jid": chat_jid, "message": f"Started {len(running_bots)} bot(s)"}
                    )
                    ...
                    for bot in bot_manager.get_available_bots():
                        ...
                        if bot_key in bot_manager.bots:
                            ...
                            stopped_count += 1
                    results["success"].append(
                        {"chat_jid": chat_jid, "message": f"Stopped {stopped_count} bot(s)"}
                    )
                    ...
                    results["failed"].append(
                        {"chat_jid": chat_jid, "error": f"Unknown action: {action}"}
                    )
        ...
        return {"message": f"Bulk action '{action}' completed", "results": results}
...
async def update_bot_settings(
    ...
    try:
        ...
        if answer_owner_messages is not None:
            success = database.set_bot_answer_owner_messages(
                bot_name, chat_jid, answer_owner_messages
            )
            if not success:
                raise HTTPException(
                    status_code=400, detail="Failed to update answer_owner_messages"
                )
        ...
        if context_message_count is not None:
            ...
            success = database.set_bot_context_message_count(
        ...
        return SuccessResponse(message=f"Updated bot settings: {', '.join(updates)}")
    except HTTPException:
        raise
    except Exception as e:
...
async def get_bot_context(bot_name: str, chat_jid: str):
    ...
    try:
        ...
        return {"bot_name": bot_name, "chat_jid": chat_jid, "context_message_count": count}
    except HTTPException:
        raise
    except Exception as e:
