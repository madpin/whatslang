...
async def verify_password(data: PasswordVerification):
    ...
    if data.password == dashboard_password:
        ...
        token = secrets.token_urlsafe(32)
        return AuthResponse(success=True, message="Authentication successful", token=token)
    else:
...
async def auth_status():
    ...
    return {"auth_required": bool(dashboard_password), "configured": bool(dashboard_password)}
