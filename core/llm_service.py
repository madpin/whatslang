...
class LLMService:
    ...
    def __init__(
        ...
        logger.info(
            f"LLMService initialized - Text: {self.model}, Vision: {self.vision_model}, Audio: {self.audio_model}"
        )
    ...
    def call_with_history(
        self, system_prompt: str, current_message: str, history: List[Dict[str, Any]]
    ) -> Optional[str]:
        """
    ...
    def call_with_image(self, prompt: str, image_bytes: bytes) -> Optional[str]:
        ...
        if len(base64_image) > 5_000_000:
            logger.warning(
                f"Image is very large ({len(base64_image)} chars base64). This may cause LLM failures."
            )
        ...
        try:
            ...
            try:
                (
                    ...
                    .output(
                        temp_audio_path, acodec="libmp3lame", ac=1, ar="16000", audio_bitrate="64k"
                    )
                    .overwrite_output()
                ...
                if (
                    "does not contain any stream" in error_message
                    or "Output file is empty" in error_message
                ):
    ...
    def transcribe_audio(self, audio_bytes: bytes, language: Optional[str] = None) -> Optional[str]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == 0:
                    logger.info(
                        f"Calling Whisper API for transcription (language: {language or 'auto-detect'})"
                    )
                else:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Audio transcription error (attempt {attempt + 1}/{max_retries}): {e}. Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                else:
