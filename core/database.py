...
class MessageDatabase:
    ...
    def list_chats(
        ...
        if bot_status_filter == "running":
            query += (
                " INNER JOIN bot_chat_assignments ON chats.chat_jid = bot_chat_assignments.chat_jid"
            )
            query += " WHERE bot_chat_assignments.running = 1"
        elif bot_status_filter == "none":
            query += (
                " LEFT JOIN bot_chat_assignments ON chats.chat_jid = bot_chat_assignments.chat_jid"
            )
    ...
    def update_message_activity(self, chat_jid: str, message_time: Optional[str] = None) -> bool:
        ...
        return self.update_chat(
            chat_jid=chat_jid, last_message_time=message_time, increment_message_count=True
        )
    ...
    def set_bot_running_state(self, bot_name: str, chat_jid: str, running: bool = True) -> bool:
    ...
    def set_bot_answer_owner_messages(
        self, bot_name: str, chat_jid: str, answer_owner_messages: bool = True
    ) -> bool:
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Set bot answer_owner_messages: {bot_name} -> {chat_jid} (answer_owner_messages={answer_owner_messages})"
            )
            return True
        except Exception as e:
    ...
    def set_bot_context_message_count(
        self, bot_name: str, chat_jid: str, context_message_count: int
    ) -> bool:
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Set bot context_message_count: {bot_name} -> {chat_jid} (count={context_message_count})"
            )
            return True
        except Exception as e:
